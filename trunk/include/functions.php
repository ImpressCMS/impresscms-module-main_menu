...
function main_menu_getModuleAdminLink($moduleName='main_menu') {
	global $icmsModule;
	...
	$ret = '';
	if ($moduleName) {
		$ret = "<a href='" . ICMS_URL . "/modules/$moduleName/admin/index.php'>" ._MD_MAIN_MENU_ADMIN_PAGE . "</a>";
	}
	return $ret;
}
 ...
 * @todo to be move in icms core
 */
function main_menu_getModuleName($withLink = true, $forBreadCrumb = false, $moduleName = false) {
	if (!$moduleName) {
		global $icmsModule;
		$moduleName = $icmsModule->getVar('dirname');
	}
 ...
 */
function main_menu_getPreviousPage($default=false) {
	global $impresscms;
 ...
 */
function main_menu_getMonthNameById($month_id) {
...
*/
function main_menu_getLinkedUnameFromId($userid, $name = false, $users = array (), $withContact = false)
{
